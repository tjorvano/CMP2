...
//AANMAKEN CUSTOM POSTTYPE
function add_my_custom_posttype(){
	//LABELS
    //VOEG HIER LABELS TOE AAN EEN ARRAY LABELS
    // VOEG DE ARRAY LABELS TOE AAN $ARGS MET labls => $labels

	//ARGUMENTEN
	$args = array(
		'label' => 'Grafisch Woordenboek',
		'Description' => 'Beschrijving van het grafische woordenboek',
		'public' => true,
		'menu_position' => 5,
		...
		'capabilities' => array(
			...
			'edit_posts'         => 'edit_feature', 
			'edit_others_posts'  => 'edit_others_feature', 
			'publish_posts'      => 'publish_feature',       
			'read_private_posts' => 'read_private_feature', 
			'create_posts'       => 'edit_feature', 
			),
	);
	register_post_type( 'woordenboek', $args );
}
//BIJ HET INITIALIZEREN DE FUNCTIE UITVOEREN
add_action( 'init', 'add_my_custom_posttype' );




//TAXONOMY AANMAKEN (een extra onderverdeling, zoals categorie)
function my_tax_studierichting_verdeling(){
	$labels = array(
		'name' => 'Woordenboek studierichtingen',
		'singular_name' => 'Woordebboek studierichting',
		'add_new_item' => 'Nieuwe richting toevoegen',
		'add_new' => 'Nieuw',
	);
	$args = array(
		'labels' => $labels,
	...
	register_taxonomy( 'studierichting', 'woordenboek', $args );
}
//BIJ HET INITIALIZEREN DE FUNCTIE UITVOEREN
add_action('init', 'my_tax_studierichting_verdeling', 0);



//METABOX AANMAKEN. EXTRA OPTIES, EXTRA VELDEN
...
function afkorting_box_woordenboek_save($post_id){
     ...
     if ( !wp_verify_nonce( $_POST['woordenboek_afk_box_content_nonce'], plugin_basename(__FILE__) )) {
      ...
      update_post_meta($post_id, '_afkorting', $afk); 
